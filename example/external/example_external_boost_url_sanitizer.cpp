...
{
  static std::array<std::pair<core::string_view, core::string_view>, 21> scheme_ports = {{{"http", "80"},
                                                                                          {"ftp", "21"},
                                                                                          {"https", "443"},
                                                                                          {"gopher", "70"},
                                                                                          {"ldap", "389"},
...
void extract_userinfo_relative(
    ...
    url_components& out)
{
  ...
  // validate port
  bool const valid_port = urls::grammar::find_if_not(
                              out.port.begin(),
                              out.port.end(),
    ...
    url_components& out)
{
  ...
  if (is_relative_ref) {
    ...
    bool const looks_like_authority = urls::parse_authority(first_seg) && host_delimiter_pos != core::string_view::npos && host_delimiter_pos != first_seg.size() - 1;
...
void print_url_components(urls::url_view u)
{
