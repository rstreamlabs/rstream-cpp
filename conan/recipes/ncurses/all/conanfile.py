...
class NCursesConan(ConanFile):
    ...
    def requirements(self):
        ...
        if self.options.get_safe("with_hashed_db") and self.settings.os in ["Linux", "FreeBSD", "Windows"]:
    ...
    def build_requirements(self):
        if cross_building(self):
            self.tool_requires('{}/{}@{}/{}'.format(self.name, self.version, self.user, self.channel), options={"with_hashed_db": self.options.get_safe("with_hashed_db")})
    ...
    def generate(self):
        ...
        tc.configure_args += [
            ...
            "--with-hashed-db={}".format((self.dependencies["libdb"].package_folder if self.settings.os in ["Linux", "FreeBSD", "Windows"] else "yes") if self.options.get_safe("with_hashed_db") else "no"),
            ...
            "--with-default-terminfo-dir=${prefix}/res/terminfo",
        ]
        build = None
        host = None
        if self.settings.os == "Windows":
    ...
    def package_info(self):
        ...
        if self.options.with_tinfo:
            ...
            if self.options.get_safe("with_hashed_db") and self.settings.os in ["Linux", "FreeBSD", "Windows"]:
                self.cpp_info.components["tinfo"].requires += [
                    "libdb::libdb"
                ]
        ...
        if self.options.get_safe("with_hashed_db"):
            terminfo = os.path.join(self.package_folder, "res", "terminfo.db")
        else:
        ...
        self.buildenv_info.define("TIC_PATH", os.path.join(self.package_folder, "bin", "tic"))
