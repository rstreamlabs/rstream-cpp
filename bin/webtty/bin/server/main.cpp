...
#include <webtty_workspace_trust.hpp>
...
int run(int argc, char** argv)
{
  ...
  if (e2e_active) {
    ...
    if (enrollment && enrollment->m_encryption_policy == "workspace_managed") {
      ...
      settings.m_client_proof_credential_verifier = [enrollment_value = *enrollment](const rstream::webtty::byte_vector& client_key_id,
        ...
        return rstream::webtty::cli::verify_workspace_client_credential(enrollment_value, client_key_id, client_public_key, credential);
      };
    }
    if (!(enrollment && enrollment->m_encryption_policy == "workspace_managed")) {
  ...
  if (use_web) {
    if (protocol_type == rstream::webtty::protocol::type::plain && !enrollment && args.at("--publish").asBool()) {
      throw std::runtime_error("lightweight plain WebTTY only supports private dialing; remove --publish or use websocket");
    }
    ...
    uri_options.m_transport         = protocol_type;
    ...
    uri_options.m_server_name       = enrollment ? enrollment->m_server_name : "";
