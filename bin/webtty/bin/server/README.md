...
Execution modes are `spawn` and `login`. Registered servers default to `login`; set `--login-user <name>` for a fixed local OS user, `--allow-client-user` only when clients are allowed to request the OS user, or `--execution-mode spawn` for the lightweight process model. C++ login mode does not handle passwords. On POSIX it switches user, primary group, and supplementary groups through the local process credentials, which requires suitable service privileges. WebTransport is not implemented in the C++ server; use the Go server for WebTransport.
