...
class WindowsAsanRequirementTest(unittest.TestCase):
    def test_cached_sdk_cannot_skip_qualification(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(len(create(["--build=missing"])), 1)
            for expected, command in enumerate(commands, 2):
                build_flags = [
                    shlex.split(flag)[0] for flag in re.findall(r"--build=\S+", command)
                ]
                with self.subTest(command=command):
                    self.assertEqual(len(create(build_flags)), expected)
