...
static void check_non_tty_file_descriptor_is_rejected()
{
  ...
  bool rejected = false;
  try {
    rstream::rtty::terminal terminal(read_end.get());
    (void)terminal;
  }
  catch (const std::system_error& error) {
    rejected = true;
    assert(error.code() == rstream::rtty::error::code::not_a_tty);
  }
  assert(rejected);
}
...
static void check_terminal_resize_and_reset_on_pty()
{
  ...
  rstream::rtty::terminal terminal(slave.get());
  rstream::rtty::terminal::size size = {
      .m_row    = 31,
      .m_col    = 111,
      .m_xpixel = 0,
      .m_ypixel = 0,
