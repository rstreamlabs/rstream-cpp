...
class stream_socket_base_interface {
 public:
  ...
  auto async_write_some(const const_buffer_sequence& buffers, BOOST_ASIO_MOVE_ARG(write_handler) handler)
  {
    return boost::asio::async_initiate<write_handler, void(const boost::system::error_code&, std::size_t)>(
        async_write_some_op<const_buffer_sequence>(*this), handler,
        buffers);
  }
  ...
  auto async_read_some(const mutable_buffer_sequence& buffers, BOOST_ASIO_MOVE_ARG(read_handler) handler)
  {
    return boost::asio::async_initiate<read_handler, void(const boost::system::error_code&, std::size_t)>(
        async_read_some_op<mutable_buffer_sequence>(*this), handler,
        buffers);
  }
 ...
 private:
    ...
    void operator()(BOOST_ASIO_MOVE_ARG(write_handler) handler, const const_buffer_sequence& buffers)
    {
      ...
      if (begin == end) {
        rstream::core::invoke_completion_handler(
            m_socket.m_executor, std::forward<write_handler>(handler),
        ...
        if (count == 1) {
          auto executor = boost::asio::get_associated_executor(handler, m_socket.m_executor);
          m_socket.async_write_some_internal(*begin, boost::asio::bind_executor(executor, std::forward<decltype(handler)>(handler)));
        }
        else {
          ...
          auto executor = boost::asio::get_associated_executor(handler, m_socket.m_executor);
          m_socket.async_write_some_internal(buffer, boost::asio::bind_executor(executor, std::forward<decltype(handler)>(handler)));
        }
      }
    }
    stream_socket_base_interface& m_socket;
  ...
  struct async_read_some_op {
    ...
    void operator()(BOOST_ASIO_MOVE_ARG(read_handler) handler, const mutable_buffer_sequence& buffers)
    {
      ...
      if (begin == end) {
        rstream::core::invoke_completion_handler(
            m_socket.m_executor, std::forward<read_handler>(handler),
            boost::system::error_code(), 0);
      }
      else {
        const auto count = boost::iterators::distance(begin, end);
        if (count == 1) {
          auto executor = boost::asio::get_associated_executor(handler, m_socket.m_executor);
          m_socket.async_read_some_internal(*begin, boost::asio::bind_executor(executor, std::forward<decltype(handler)>(handler)));
        }
        else {
          ...
          auto executor = boost::asio::get_associated_executor(handler, m_socket.m_executor);
          m_socket.async_read_some_internal(buffer, boost::asio::bind_executor(executor, std::forward<decltype(handler)>(handler)));
        }
      }
    }
    stream_socket_base_interface& m_socket;
...
class stream_socket_base : public socket_base<T>, public stream_socket_base_interface {
 public:
  ...
  auto async_connect(const endpoint_type& endpoint, BOOST_ASIO_MOVE_ARG(connect_handler) handler)
  {
 ...
 private:
  struct async_connect_op {
    ...
    {
      auto executor = boost::asio::get_associated_executor(handler, m_socket.get_executor());
      m_socket.async_connect_internal(endpoint, boost::asio::bind_executor(executor, std::forward<decltype(handler)>(handler)));
    }
