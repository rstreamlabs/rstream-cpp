...
static bool has_client_certificate_config(const io::address& server_address)
{
  for (const auto param : io::detail::stream::url_params(server_address.m_url)) {
...
class RSTREAM_GNUC_INTERNAL client::impl : public std::enable_shared_from_this<impl> {
 ...
 private:
  ...
  struct connect_op_type {
    using ptr = std::shared_ptr<connect_op_type>;
  ...
  void cancel_accept_tunnel(const tunnel_type::id& tunnel_id, const accept_tunnel_op_type::ptr& op);

  void complete_connect(const connect_op_type::ptr& op, const boost::system::error_code& error_code);

  void complete_create_tunnel(const create_tunnel_op_type::ptr& op, const boost::system::error_code& error_code, tunnel tunnel);

  void complete_accept_tunnel(const accept_tunnel_op_type::ptr& op, const boost::system::error_code& error_code);

  void close_tunnel_internal(const tunnel_type::id& tunnel_id);

  void do_resolve_host();

  void on_do_resolve_host(generation_type generation, const boost::system::error_code& error_code, const resolver_type::results_type& results);

  void do_connect(const resolver_type::results_type& results);

  void on_connect(generation_type generation, const boost::system::error_code& error_code, const resolver_type::results_type::endpoint_type& endpoint);
  ...
  void on_connect_stream(generation_type generation, const tunnel_type::id& tunnel_id, const stream_type::id& stream_id, const boost::system::error_code& error_code);

  void do_read_incoming_message();

  void on_read_incoming_data(generation_type generation, const boost::system::error_code& error_code);

  void on_read_incoming_message(generation_type generation, const protobuf::Message& message);
  ...
  void on_send_message(generation_type generation, const boost::system::error_code& error_code, const on_send_message_cb& cb);
  ...
  void on_queue_cancelled(
      const boost::system::error_code& error_code,
      const boost::system::error_code& cause,
      state previous_state,
      on_disconnection_cb disconnection_callback);
  ...
  boost::optional<std::string> m_default_server_address;

  boost::system::error_code m_default_server_error;

  boost::optional<std::string> m_token;

  boost::system::error_code m_token_error;

  boost::optional<std::string> m_mtls_token;

  boost::system::error_code m_mtls_token_error;
  ...
  connect_op_type::ptr m_connect_op;
      ...
      m_buffer(allocator)
{
  ...
  if (server_result) {
    m_default_server_address = server_result.value();
  }
  else {
    m_default_server_error = server_result.error();
  }
  const auto token_result = get_rstream_token(config, io::make_address("tcp://localhost"));
  if (token_result) {
    m_token = token_result.value();
  }
  else {
    m_token_error = token_result.error();
  }
  const auto mtls_token_result = get_rstream_token(config, io::make_address("tcp://localhost?ssl&ssl.cert_file=snapshot"));
  if (mtls_token_result) {
    m_mtls_token = mtls_token_result.value();
  }
  else {
    m_mtls_token_error = mtls_token_result.error();
  }
}
...
{
  if (m_default_server_address) {
    return async_connect(m_default_server_address.get(), std::move(handler));
  }
  const auto error_code = m_default_server_error ? m_default_server_error : error::make_error_code(error::code::invalid_configuration);
...
void client::impl::async_create_tunnel(const tunnel_properties& properties, async_create_tunnel_completion_handler&& handler)
{
  ...
  if (cancellation_slot.is_connected()) {
    const std::weak_ptr<impl> weak_self                = shared_from_this();
    const std::weak_ptr<create_tunnel_op_type> weak_op = op;
    cancellation_slot.assign([weak_self, weak_op](boost::asio::cancellation_type type) {
      if (type == boost::asio::cancellation_type::none) {
        return;
      }
      ...
      const auto self = weak_self.lock();
      if (self) {
        boost::asio::dispatch(self->m_strand, [self, op] { self->cancel_create_tunnel(op); });
      }
    });
  }
  boost::asio::dispatch(m_strand, [self = shared_from_this(), op] { self->async_create_tunnel_internal(op); });
}
...
void client::impl::async_accept_tunnel(const std::string& tunnel_id, socket& peer, endpoint& endpoint, tunnel::async_accept_completion_handler&& handler)
{
  ...
  if (cancellation_slot.is_connected()) {
    const std::weak_ptr<impl> weak_self                = shared_from_this();
    const std::weak_ptr<accept_tunnel_op_type> weak_op = op;
    cancellation_slot.assign([weak_self, weak_op, tunnel_id](boost::asio::cancellation_type type) {
      if (type == boost::asio::cancellation_type::none) {
        return;
      }
      const auto op = weak_op.lock();
      ...
      const auto self = weak_self.lock();
      if (self) {
        boost::asio::dispatch(self->m_strand, [self, tunnel_id, op] { self->cancel_accept_tunnel(tunnel_id, op); });
      }
    });
  }
  boost::asio::dispatch(m_strand, [self = shared_from_this(), tunnel_id, op] { self->async_accept_tunnel_internal(tunnel_id, op); });
}
...
void client::impl::arm_state_timer(unsigned int timeout_ms)
{
  ...
  auto on_timer_cb         = [task_ptr, ptr, generation](const boost::system::error_code& error_code) {
    if (task_ptr->m_complete) {
      return;
    }
...
void client::impl::complete_create_tunnel(const create_tunnel_op_type::ptr& op, const boost::system::error_code& error_code, tunnel tunnel)
{
  if (!op || op->m_completed) {
    return;
  }
  op->m_completed = true;
  rstream::core::invoke_completion_handler(m_executor, std::move(op->m_handler), error_code, std::move(tunnel));
  op->m_handler = nullptr;
}

void client::impl::complete_accept_tunnel(const accept_tunnel_op_type::ptr& op, const boost::system::error_code& error_code)
{
  if (!op || op->m_completed) {
    return;
  }
...
void client::impl::cancel_connect(const connect_op_type::ptr& op)
{
...
void client::impl::cancel_create_tunnel(const create_tunnel_op_type::ptr& op)
{
  ...
  complete_create_tunnel(op, boost::asio::error::operation_aborted, nullptr);
}

void client::impl::cancel_accept_tunnel(const tunnel_type::id& tunnel_id, const accept_tunnel_op_type::ptr& op)
{
  if (!op || !op->m_cancelled.load(std::memory_order_acquire) || op->m_completed) {
    return;
  }
  const auto tunnel_it = m_tunnels.find(tunnel_id);
  if (tunnel_it == m_tunnels.end() || tunnel_it->second->m_accept_tunnel_op != op) {
    return;
  }
  if (op->m_busy) {
    boost::system::error_code error_code;
    op->m_peer.close(error_code);
  }
  tunnel_it->second->m_accept_tunnel_op = nullptr;
...
void client::impl::async_create_tunnel_internal(const create_tunnel_op_type::ptr& op)
{
#ifdef DEBUG_BUILD
  assert(m_strand.running_in_this_thread());
#endif
  ...
  if (op->m_cancelled.load(std::memory_order_acquire)) {
    ...
    complete_create_tunnel(op, error::code::invalid_state, nullptr);
  }
  else {
    if ((op->m_properties.m_type && op->m_properties.m_type.value() != "bytestream") || invalid_published_tcp_options(op->m_properties)) {
...
void client::impl::async_accept_tunnel_internal(const tunnel_type::id& tunnel_id, const accept_tunnel_op_type::ptr& op)
{
#ifdef DEBUG_BUILD
  assert(m_strand.running_in_this_thread());
#endif
  ...
  if (error_code) {
    complete_accept_tunnel(op, error_code);
  }
  else {
    if (op->m_peer.m_impl) {
#ifdef DEBUG_BUILD
    ...
    if (stream_it != tunnel_it->second->m_streams.end()) {
      op->m_peer.adopt_impl(std::move(stream_it->second.m_ptr->m_peer));
      ...
      op->m_endpoint     = rstream::io_rstrm::endpoint{
          .m_id_name                       = stream_it->first,
          .m_server_address                = address,
          .m_server_address_from_uri_param = false,
          .m_secret                        = boost::none,
          .m_source_ip                     = stream_it->second.m_context.m_source_ip,
      };
      complete_accept_tunnel(op, boost::system::error_code());
#ifdef DEBUG_BUILD
      ...
      tunnel_it->second->m_accept_tunnel_op = op;
      if (!m_config.m_async_stream_operation) {
        for (stream_it = tunnel_it->second->m_streams.begin(); stream_it != tunnel_it->second->m_streams.end(); ++stream_it) {
...
void client::impl::do_resolve_host()
{
  ...
  assert(m_strand.running_in_this_thread());
#endif
  auto completion_handler = std::bind(&impl::on_do_resolve_host, shared_from_this(), m_generation, std::placeholders::_1, std::placeholders::_2);
#ifdef RSTREAM_WITH_IO_STREAMS
...
void client::impl::on_do_resolve_host(generation_type generation, const boost::system::error_code& error_code, const resolver_type::results_type& results)
{
#ifdef DEBUG_BUILD
  assert(m_strand.running_in_this_thread());
#endif
  if (generation != m_generation || m_state != state::connecting) {
    return;
  }
...
void client::impl::do_connect(const resolver_type::results_type& results)
{
  ...
  assert(m_strand.running_in_this_thread());
#endif
  auto completion_handler = std::bind(&impl::on_connect, shared_from_this(), m_generation, std::placeholders::_1, std::placeholders::_2);
  boost::asio::async_connect(m_socket, results, boost::asio::bind_executor(m_strand, completion_handler));
}

void client::impl::on_connect(generation_type generation, const boost::system::error_code& error_code, const resolver_type::results_type::endpoint_type&)
{
#ifdef DEBUG_BUILD
  assert(m_strand.running_in_this_thread());
#endif
...
boost::system::result<client_details> client::impl::effective_client_details() const
{
  if (has_client_certificate_config(m_server_address)) {
    if (m_mtls_token_error) {
      return m_mtls_token_error;
    }
    return get_client_details(m_mtls_token);
  }
  if (m_token_error) {
    return m_token_error;
  }
  return get_client_details(m_token);
}
...
void client::impl::do_open()
{
  ...
  boost::system::error_code error_code;
  {
    ...
    auto client_details = effective_client_details();
    if (client_details) {
...
void client::impl::on_tunnel_response(create_tunnel_ops_type::iterator& create_tunnel_op_it, const tunnel_properties& tunnel_properties, const boost::system::error_code& error_code)
{
  ...
  tunnel_type::id tunnel_id;
  if (!error_code) {
    tunnel_id     = tunnel_properties.m_id.get();
    auto endpoint = rstream::io_rstrm::endpoint{
        .m_id_name                       = tunnel_id,
        .m_server_address                = m_server_address,
        .m_server_address_from_uri_param = false,
        .m_secret                        = boost::none,
  ...
  const auto op = create_tunnel_op_it->second;
  if (op->m_cancelled.load(std::memory_order_acquire)) {
    complete_create_tunnel(op, boost::asio::error::operation_aborted, nullptr);
    if (!error_code) {
      close_tunnel_internal(tunnel_id);
    }
  }
  else {
    complete_create_tunnel(op, error_code, std::move(tunnel));
  }
...
void client::impl::on_tunnel_close(tunnels_type::iterator& tunnel_it)
{
  ...
  if (tunnel_it->second->m_accept_tunnel_op) {
    ...
    complete_accept_tunnel(tunnel_it->second->m_accept_tunnel_op, error::code::tunnel_not_found);
  }
...
void client::impl::do_connect_stream(tunnels_type::iterator& tunnel_it, streams_type::iterator& stream_it)
{
  ...
  if (peer) {
    ...
    endpoint endpoint   = {
        ...
        .m_source_ip                     = context.m_source_ip,
    };
    auto completion_handler = std::bind(&impl::on_connect_stream, shared_from_this(), m_generation, tunnel_it->first, stream_it->first, std::placeholders::_1);
    boost::system::error_code error_code;
    if (!error_code) {
    ...
    if (!error_code) {
      ...
      on_connect_stream(m_generation, tunnel_it->first, stream_it->first, error_code);
    }
  }
  else {
#ifdef DEBUG_BUILD
...
void client::impl::on_connect_stream(generation_type generation, const tunnel_type::id& tunnel_id, const stream_type::id& stream_id, const boost::system::error_code& error_code)
{
#ifdef DEBUG_BUILD
  assert(m_strand.running_in_this_thread());
#endif
  if (generation != m_generation || m_state != state::connected) {
    return;
  }
  tunnels_type::value_type::second_type tunnel;
  {
  ...
  if (error_code) {
    ...
    if (tunnel->m_accept_tunnel_op) {
      if (it->second.m_ptr) {
        tunnel->m_accept_tunnel_op->m_peer.adopt_impl(std::move(it->second.m_ptr->m_peer));
      }
      const auto address                     = it->second.m_context.m_proxy_endpoint ? it->second.m_context.m_proxy_endpoint.get() : m_server_address;
      tunnel->m_accept_tunnel_op->m_endpoint = rstream::io_rstrm::endpoint{
          .m_id_name                       = it->first,
          .m_server_address                = address,
          .m_server_address_from_uri_param = false,
          .m_secret                        = boost::none,
          .m_source_ip                     = it->second.m_context.m_source_ip,
      };
      complete_accept_tunnel(tunnel->m_accept_tunnel_op, boost::system::error_code());
      it                         = tunnel->m_streams.erase(it);
...
void client::impl::do_read_incoming_message()
{
  ...
  m_buffer.reset_size();
  auto completion_handler = std::bind(&impl::on_read_incoming_data, shared_from_this(), m_generation, std::placeholders::_1);
  m_payloader.async_recv(m_buffer, boost::asio::bind_executor(m_strand, completion_handler));
}

void client::impl::on_read_incoming_data(generation_type generation, const boost::system::error_code& error_code)
{
#ifdef DEBUG_BUILD
  assert(m_strand.running_in_this_thread());
#endif
  if (generation != m_generation || m_state == state::null) {
    return;
  }
  if (error_code) {
    on_error(error_code);
...
void client::impl::on_read_incoming_message(generation_type generation, const protobuf::Message& message)
{
#ifdef DEBUG_BUILD
  assert(m_strand.running_in_this_thread());
#endif
  ...
  if (!is_message_expected(m_state, message)) {
    ...
    if (message_type == payload_type::kOpenControlChannelRsp) {
      const auto& payload = message.open_control_channel_rsp();
      if (payload.has_ok()) {
      ...
      {
        const auto& payload = message.open_tunnel_rsp();
      ...
      tunnels_type::iterator it;
      {
        const auto& payload = message.close_tunnel_rsp();
      ...
      {
        const auto& payload = message.proxy_conn_req();
...
void client::impl::do_send_message(const protobuf::Message& message, const on_send_message_cb& cb)
{
  ...
  auto completion_handler = std::bind(&impl::on_send_message, shared_from_this(), m_generation, std::placeholders::_1, cb);
  m_queue.async_send(buffer, boost::asio::bind_executor(m_strand, completion_handler));
}

void client::impl::on_send_message(generation_type generation, const boost::system::error_code& error_code, const on_send_message_cb& cb)
{
#ifdef DEBUG_BUILD
  assert(m_strand.running_in_this_thread());
#endif
...
void client::impl::on_close(const boost::system::error_code& error_code)
{
  ...
  assert(m_strand.running_in_this_thread());
#endif
  if (m_state == state::null || m_close_pending) {
    return;
  }
  ...
  const auto previous_state = m_state;
  on_disconnection_cb disconnection_callback;
  {
    std::lock_guard<std::mutex> lock(m_mutex);
  ...
  auto completion_handler = [self, cause, previous_state, disconnection_callback = std::move(disconnection_callback)](
                                const boost::system::error_code& queue_error) mutable {
    self->on_queue_cancelled(queue_error, cause, previous_state, std::move(disconnection_callback));
  };
  m_queue.async_cancel(boost::asio::bind_executor(m_strand, std::move(completion_handler)));
}

void client::impl::on_queue_cancelled(
    const boost::system::error_code& error_code,
    const boost::system::error_code& cause,
    state previous_state,
    on_disconnection_cb disconnection_callback)
{
  ...
  boost::system::error_code close_cause = cause;
  if (!close_cause && error_code) {
    close_cause = error_code;
  }
  {
  ...
  for (auto it = m_create_tunnel_ops.begin(); it != m_create_tunnel_ops.end();) {
    on_tunnel_response(it, tunnel_properties(), close_cause ? close_cause : error::code::operation_aborted);
  }
  for (auto it = m_tunnels.begin(); it != m_tunnels.end();) {
    on_tunnel_close(it);
  }
  if (previous_state == state::connecting) {
    complete_connect(m_connect_op, close_cause ? close_cause : error::code::operation_aborted);
  }
  m_connect_op = nullptr;
  if ((previous_state == state::connected || previous_state == state::closing) && disconnection_callback) {
    disconnection_callback(close_cause);
  }
#ifdef DEBUG_BUILD
